// lib/agents/tools/insurance/insurers/bajaj-allianz-life.ts
...
async function discover(html: string, monthOpt: string | "latest"): Promise<DiscoveredFactsheet | null> {
  ...
  // Bajaj Allianz Life publishes monthly "Investment Insight" PDFs on
  // /our-funds/fund-performance.html. Find the topmost <a href ending in .pdf>
  // whose text or href mentions "Investment Insight" or "factsheet".
  const candidates: Array<{ url: string; label: string }> = [];
  $('a[href$=".pdf"]').each((_, el) => {
    const href = $(el).attr("href") ?? "";
    const label = ($(el).text() || "").trim();
    if (!/(investment|factsheet|fund)/i.test(`${href} ${label}`)) return;
    const abs = href.startsWith("http") ? href : `https://www.bajajallianzlife.com${href.startsWith("/") ? "" : "/"}${href}`;
...
export const bajajAllianzLifeEntry: InsurerCatalogEntry = {
  slug: "bajaj-allianz-life",
  displayName: "Bajaj Allianz Life",
  domain: "bajajallianzlife.com",
  landingUrl: "https://www.bajajallianzlife.com/our-funds/fund-performance.html",
  discover,
