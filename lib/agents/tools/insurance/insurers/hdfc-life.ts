// lib/agents/tools/insurance/insurers/hdfc-life.ts
...
async function discover(html: string, monthOpt: string | "latest"): Promise<DiscoveredFactsheet | null> {
  ...
  $('a[href$=".pdf"]').each((_, el) => {
    ...
    if (!/(factsheet|fund-performance|investment)/i.test(`${href} ${label}`)) return;
    const abs = href.startsWith("http") ? href : `https://www.hdfclife.com${href.startsWith("/") ? "" : "/"}${href}`;
...
export const hdfcLifeEntry: InsurerCatalogEntry = {
  slug: "hdfc-life",
  displayName: "HDFC Life",
  domain: "hdfclife.com",
  landingUrl: "https://www.hdfclife.com/our-products/savings-and-investment-plans/fund-performance",
  discover,
