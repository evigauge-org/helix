// lib/agents/tools/insurance/insurers/sbi-life.ts
...
async function discover(html: string, monthOpt: string | "latest"): Promise<DiscoveredFactsheet | null> {
  ...
  $('a[href$=".pdf"]').each((_, el) => {
    ...
    if (!/(factsheet|fund-performance|investor|monthly)/i.test(`${href} ${label}`)) return;
    const abs = href.startsWith("http") ? href : `https://www.sbilife.co.in${href.startsWith("/") ? "" : "/"}${href}`;
...
export const sbiLifeEntry: InsurerCatalogEntry = {
  slug: "sbi-life",
  displayName: "SBI Life",
  domain: "sbilife.co.in",
  landingUrl: "https://www.sbilife.co.in/en/about-us/investor-relations/fund-performance",
  discover,
