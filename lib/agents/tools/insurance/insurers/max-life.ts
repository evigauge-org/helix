// lib/agents/tools/insurance/insurers/max-life.ts
...
async function discover(html: string, monthOpt: string | "latest"): Promise<DiscoveredFactsheet | null> {
  ...
  $('a[href$=".pdf"]').each((_, el) => {
    ...
    if (!/(factsheet|fund-performance|investor)/i.test(`${href} ${label}`)) return;
    const abs = href.startsWith("http") ? href : `https://www.maxlifeinsurance.com${href.startsWith("/") ? "" : "/"}${href}`;
...
export const maxLifeEntry: InsurerCatalogEntry = {
  slug: "max-life",
  displayName: "Max Life Insurance",
  domain: "maxlifeinsurance.com",
  landingUrl: "https://www.maxlifeinsurance.com/about-us/investor-relations/fund-performance",
  discover,
