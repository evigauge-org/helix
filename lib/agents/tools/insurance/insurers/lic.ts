// lib/agents/tools/insurance/insurers/lic.ts
// CAVEAT: licindia.in is JS-rendered. discover() will likely return null because the funds page is hydrated client-side. The fetch tool falls through to genericFallbackDiscover (Exa) automatically. Do not waste time tweaking selectors here — solve via Playwright in v1.1 if needed.
...
async function discover(html: string, monthOpt: string | "latest"): Promise<DiscoveredFactsheet | null> {
  ...
  $('a[href$=".pdf"]').each((_, el) => {
    ...
    if (!/(factsheet|fund|nav|monthly)/i.test(`${href} ${label}`)) return;
    const abs = href.startsWith("http") ? href : `https://licindia.in${href.startsWith("/") ? "" : "/"}${href}`;
...
export const licEntry: InsurerCatalogEntry = {
  slug: "lic",
  displayName: "LIC (Life Insurance Corporation of India)",
  domain: "licindia.in",
  landingUrl: "https://licindia.in/funds-management/fund-performance",
  discover,
