// lib/agents/tools/insurance/insurers/tata-aia-life.ts
...
async function discover(html: string, monthOpt: string | "latest"): Promise<DiscoveredFactsheet | null> {
  ...
  // Tata AIA's funds page lists factsheets under a "Fund Factsheet" accordion.
  // Iterate every PDF link; filter to those whose href or surrounding text
  // mentions "factsheet"/"investment".
  const candidates: Array<{ url: string; label: string }> = [];
  $('a[href$=".pdf"]').each((_, el) => {
    const href = $(el).attr("href") ?? "";
    const label = ($(el).text() || "").trim();
    if (!/(factsheet|investment|fund-performance|fund_performance)/i.test(`${href} ${label}`)) return;
    const abs = href.startsWith("http") ? href : `https://www.tataaia.com${href.startsWith("/") ? "" : "/"}${href}`;
...
export const tataAiaLifeEntry: InsurerCatalogEntry = {
  slug: "tata-aia-life",
  displayName: "Tata AIA Life",
  domain: "tataaia.com",
  landingUrl: "https://www.tataaia.com/insurance-plans/investments/funds.html",
  discover,
